...
from parser_storedriver.google_play_scraper_master.google_play_scraper.util import PlayStoreException, PlayStoreCollections
...
class PlayStoreScraper:
	...
	def get_app_ids_for_developer(self, developer_id, num=60, country="nl", lang="nl"):
		...
		try:
			...
			result = requests.get(url.replace('developer', 'dev')).text
			data = self.extract_json_block(result, "ds:3")
			data = json.loads(data)
	...
	def get_app_details(self, app_id, country="nl", lang="nl"):
		...
		try:
			site_developer = info[0][12][5][3][5][2]
		except:
			site_developer = None
		app = {
			...
			"os": version[2],
			"software": version[1],
			"email_developer": info[0][12][5][2][0],
